...
def main() -> None:
    ...
    print(
        f"Generated {date.today().isoformat()}. Tolerances are the published ones: EBU Tech 3341 ±0.1 LU for loudness and +0.2/−0.4 dB for true peak; EBU Tech 3342 ±1 LU for loudness range; TASA Standard §1.4.2 per-frequency tolerances for the M-weighting. The EBU Loudness Test Set v5.0 is free from tech.ebu.ch and is not redistributed here.\n"
    )
    if not ROOT.exists():
    ...
    for freq, phase in [
        ...
        print(
            f"| {freq / 1000:g} kHz | {sp:+.2f} | {a.result().true_peak_dbtp:+.2f} | {b.result().true_peak_dbtp:+.2f} | 0.00 |"
        )
    print()
    ...
    okc = 0
    for (f, target, tol), r in zip(pts, resp, strict=True):
