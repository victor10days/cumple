...
- **Tests**: 229, 91 % line coverage on `src/`, run with `uv run pytest`. CI
  runs 200 of them on Ubuntu, 16 fewer on macOS, which has no ffmpeg for the
  decode tests, and 17 fewer on Windows, on every push. The 29 EBU cases
