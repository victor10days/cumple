...
Everything streams: memory grows far slower than the file (five minutes of
stereo peak at 428 MB, sixty minutes at 704 MB,
...
with cumple and runs offline through onnxruntime, on a platform onnxruntime
supports (there is no macOS Intel wheel for 1.30): install with
...
- **Benchmark**: the same files through libebur128, pyloudnorm, ffmpeg's
  `ebur128` and loudcheck, side by side, every reading scored against the
  published tolerance. [docs/BENCHMARK.md](docs/BENCHMARK.md), regenerated by
  `scripts/benchmark_meters.py`.
...
- **Real dialogue**: the speech gate against two open films that publish a
  ...
  dialogue, with Silero VAD, the `--vad silero` backend, beside it.
  [docs/DIALOGUE.md](docs/DIALOGUE.md), regenerated by
  `scripts/dialogue_benchmark.py` after `scripts/fetch_real_dialogue.sh`.
- **Tests**: 252, 91 % line coverage on `src/`, run with `uv run pytest`. CI
  runs 223 of them on Ubuntu, 19 fewer on macOS, which has no ffmpeg for the
  decode tests, and 20 fewer on Windows, on every push. The 29 EBU cases
  run when the test set is in `~/.cache/cumple/` (it is free but not
  redistributed; see CONTRIBUTING), and in a separate CI job whenever the
  repository holds a private link to a copy of the set; without one that job
  reports them as skipped. As of 8 September 2026 no link is set, so that job
  has only ever reported skips.
- **Hands-on checks** are logged in [docs/QA.md](docs/QA.md): what has run on
  the synthetic demo set, through the app's own bridge or the command line,
  and what still needs real bounces, a DAW or a Windows or Linux machine.

The conformance and performance reports are headed cumple 0.2.0 and were
generated on 8 September 2026; the benchmark and the dialogue report are
headed 0.2.1, generated on 12 and 13 September. The meters are unchanged
since 0.2.0, so the four describe the same code.
...
- The dialogue gate is a heuristic (band energy, syllabic modulation, an
  ...
  `--vad silero`, reads 0 % speech on the music-and-effects versions and
  reads 1.2 LU low on Tears of Steel, but on Sintel, whose
  ...
  it. Because reading low turns a too-loud mix into a pass, a dialogue-gated
  pass that sits 3 LU or more under the full-programme value is reported as a
  warning that asks for a Dolby meter before sending, on destinations whose
  window the full-programme value exceeds (a tool default; both films' true
  values sat within 3 LU of the full programme, the detector's 2.8 and 9.5 LU
  under; that the full programme caps the dialogue is an assumption drawn from
  those two films).
...
- Reads what libsndfile reads: WAV, BWF, RF64, AIFF, FLAC and MP3. With a
  ...
  decodes AAC, AC-3 and E-AC-3 and the audio of MXF, MOV and MP4 files that
  carry one audio stream, so a compressed delivery is measured and its container
  fails the destination's clause instead of the tool refusing to open it; the
  report names the decoder.
...
[libebur128](https://github.com/jiixyj/libebur128) is an open-source library
for momentary, short-term and integrated loudness, loudness range and true
peak, with no command line of its own;
[pyloudnorm](https://github.com/csteinmetz1/pyloudnorm) measures integrated
loudness and loudness range in Python (whole file in memory, no true peak);
ffmpeg's `ebur128` filter is an independent implementation;
[loudcheck](https://github.com/chaoz23/loudcheck) gives pass/fail verdicts for
EBU R 128 and ATSC A/85 through ffmpeg's loudnorm filter. All four were run on
the EBU files beside cumple: [docs/BENCHMARK.md](docs/BENCHMARK.md).
[bbc/audio-offset-finder](https://github.com/bbc/audio-offset-finder) finds
offsets between recordings; [DeltaWave](https://deltaw.org/) is free Windows
...
Beyond the five tools run here, [docs/RELATED.md](docs/RELATED.md) now records
three more open-source wrappers run on the EBU files on 12 September 2026
(ffmpeg-normalize, r128gain and rsgain, all inside tolerance, each re-measuring
an engine already in the table: ffmpeg loudnorm, ffmpeg's ebur128 filter and
libebur128), twelve commercial meters and nine file-based QC platforms read
from their documentation, what they list that cumple lacks (a dialogue gate of
the Dolby Dialogue Intelligence class, delivery codec and container
conformance, Dolby E detection, and ADM and Atmos metadata checks) and what
cumple has that none of the 21 list: a quoted clause with a source grade behind
every verdict, a stem-null diff in words, and offline use with no account under
MIT. cumple combines what none of them do together: named destinations with
graded sources, verdicts with the clause attached, a report that travels with
the delivery, and a diff in words.
