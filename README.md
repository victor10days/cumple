...
- **Tests**: 185, 91 % line coverage on `src/`, run with `uv run pytest`. CI
  runs 156 of them on Ubuntu, macOS and Windows on every push. The 29 EBU cases
...
- The dialogue gate is a heuristic (band energy, syllabic modulation, an
  ...
  warning that asks for a Dolby meter before sending (a tool default; both
  films' true values sat within 3 LU, the detector's 2.8 and 9.5 LU under).
